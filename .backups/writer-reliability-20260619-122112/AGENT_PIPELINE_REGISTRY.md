...
| 2.6 | If yes → **run `publish.sh --status draft` directly in bash** (no wp-publisher spawn) → `update_recent_topics.py --status drafted` → `build_and_send_card.py` → `update_pick_status.py --status published` |
...
- **creator:** Generate feature image from `validated.json` using Scene Formula in SOUL (scene tables in `generate-image/SKILL.md`); run `generate.sh`.
