...
1. Build the with-image markdown + DOCX, then verify:
   ...
   ```bash
   # Resolve per-project Drive settings (same values Press used to read).
   DRIVE_PREFIX=$(python3 ~/.openclaw/workspace-orchestrator/skills/pipeline/project_config.py --path "$PROJECT_CONFIG" --field publisher.drive_doc_prefix)
   DRIVE_PARENT=$(python3 ~/.openclaw/workspace-orchestrator/skills/pipeline/project_config.py --path "$PROJECT_CONFIG" --field publisher.drive_parent_id)
   DRIVE_ACCT=$(python3 ~/.openclaw/workspace-orchestrator/skills/pipeline/project_config.py --path "$PROJECT_CONFIG" --field publisher.drive_account)
...
3. Upload to Google Drive (do **not** use `--convert` on a .docx — it strips the embedded image):
   ...
   ```bash
   DRIVE_JSON=$(GOG_KEYRING_PASSWORD="sawan" gog drive upload "$ARTICLE_DOCX" \
