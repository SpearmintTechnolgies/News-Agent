...
Run this **exact** command using your bash tool. Replace `<YOUR CRAFTED PROMPT>` with the prompt you crafted in Step 1, and `<SAVE PATH FROM ORCHESTRATOR MESSAGE>` with the exact `feature.jpg` path the orchestrator told you to save to (e.g. `/tmp/<slug>-run-.../media/feature.jpg`). Setting `OUTPUT_PATH` keeps concurrent runs from overwriting each other's image:

```bash
OUTPUT_PATH="<SAVE PATH FROM ORCHESTRATOR MESSAGE>" \
  bash ~/.openclaw/workspace-creator/skills/generate-image/generate.sh "<YOUR CRAFTED PROMPT>"
```

If the orchestrator message did not give an explicit path, omit `OUTPUT_PATH=` and the script falls back to its default.

**WAIT for the script to finish. Do NOT skip this step. Do NOT guess the output. Do NOT run generate.sh twice in parallel for the same project. Never `pkill` a running generate.sh.**

On failure the reason is in `/tmp/<slug>-image-error.log` (legacy mirror: `/tmp/image-error.log`). If it contains `IMAGE_BUSY`, poll the existing process until it exits, then verify with Step 3 — do **not** re-run generate.sh.
...
Run this bash command to verify the final file. Use the SAME save path you passed as `OUTPUT_PATH` (resolves symlinks):
```bash
FEATURE_IMG="$(readlink -f "<SAVE PATH FROM ORCHESTRATOR MESSAGE>" 2>/dev/null || echo "<SAVE PATH FROM ORCHESTRATOR MESSAGE>")"
...
Return exactly the save path the orchestrator gave you (e.g. `<SAVE PATH FROM ORCHESTRATOR MESSAGE>`).

If the Step 3 bash command outputs `INVALID_IMAGE`:
Run `cat /tmp/<slug>-image-error.log` (or the legacy `/tmp/image-error.log`) to see what went wrong, and then return exactly:
