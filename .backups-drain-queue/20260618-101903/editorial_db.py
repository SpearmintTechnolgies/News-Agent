...
def count_queued_jobs(*, db_path: str = DEFAULT_DB_PATH) -> int:
    init_db(db_path)
    with _connect(db_path) as conn:
        row = conn.execute(
...
def claim_next_feed_job(
    *, max_concurrent: int = 1, db_path: str = DEFAULT_DB_PATH
) -> FeedJob | None:
    """Atomically promote the oldest eligible queued job to running.

    Concurrency model (default ``max_concurrent=1`` = fully serial, unchanged
    behavior): at most ``max_concurrent`` jobs run at once AND at most one job
    per project runs at a time. This lets different publications run in parallel
    (the goal) while keeping same-project runs serial — which is what makes the
    per-project ``/tmp/<slug>-*`` scratch files safe under concurrency.
    """
    ...
    with _connect(db_path) as conn:
        ...
        # Oldest queued job whose project is not already running.
        candidate = None
    ...
    return _row_to_feed_job(fresh) if fresh else None
