...
| Feed-card callbacks (`oc_go:`,`oc_feed_refresh:`) | FEEDBACK | — | — | **EDITORIAL_FEEDBACK.md** (single-click `oc_go` enqueues a job; dispatcher starts the pipeline) |
...
| Message starts with `FEED_JOB ` (from the feed-job dispatcher cron) | FEED_JOB | classify-only | ON | **Single feed-job entry** |
...
All human/agent contact happens in the **project's Telegram group** (`telegram.group_id` in the project config; captured as `$GROUP_CHAT_ID` at Step 0), never DM. Coinography -> `-1003760909509`; MemeCoinist -> `-5532153816`. Each group is one isolated session.
...
- Message starts with `FEED_JOB ` → jump to **Single feed-job entry** (do not run Step 0.0 greeting logic).
...
GROUP ROUTING RULE: Each publication has its OWN Telegram group (`telegram.group_id` in `$PROJECT_CONFIG`). Every group-facing message in this run — the Step 2.5 gate, Step 2.6 success/skip replies, cap notices, mini-reports, and the "feed job finished" note — MUST be sent to `$GROUP_CHAT_ID` using the `message` tool (target the explicit chat id). Do NOT rely on implicit "reply in the current chat" delivery: FEED_JOB and AUTO_RUN runs execute in isolated cron sessions with no inbound group context, so an unaddressed reply would land in the wrong group.
...
### Single feed-job entry (hourly single-click feed card)

Reached when the incoming message starts with `FEED_JOB ` followed by JSON like `{"job_id":12,"project":"coinography","feed_id":"f-coinography-...","candidate_index":2,"selection_file":"/tmp/openclaw-feed-go-....json"}` (delivered by `dispatch_feed_jobs.py` one-shot cron). Parse the JSON.

1. Set: `FEED_JOB_ID=<job_id>`, `PROJECT_SLUG=<project>`, `SELECTION_FILE=<selection_file>`, `PICKER_MODE=classify-only`, `STEP25_GATE=ON`, `CLASSIFY_ONLY_FLAG="--classify-only"`, `N=1`.
2. Run Step 0, Step 0.5 (`N=1`), Step 1 (classify-only branch, using `$SELECTION_FILE`), Step 1c, then the Step 2 queue (STEP25_GATE=ON), then Step 3.
3. **Always** mark the job complete so the queue can drain (even on fatal stop — use `failed`):
   ```bash
   python3 ~/.openclaw/workspace-orchestrator/skills/pipeline/mark_feed_job.py \
     --job-id "$FEED_JOB_ID" --status done
   ```
   On a fatal stop before Step 3, run the same with `--status failed` instead.
4. Post a one-line note to the project's group `$GROUP_CHAT_ID` (via the `message` tool): "Feed job $FEED_JOB_ID finished on $PROJECT_SLUG." Then end the turn — do not wait for input beyond any Step 2.5 gate inside the run.
...
2. Run Step 0, Step 0.5 (`N`), Step 1 (select branch, `--pool-fresh 15`, target=N), Step 1c, then the Step 2 queue with **STEP25_GATE=OFF** (publish straight to draft + card, no per-story gate), and the daily-cap check at Step 2.6.
