...
2. Run Step 0, Step 0.5 (`N=1`), Step 1 (classify-only branch, using `$SELECTION_FILE`), Step 1c, then the Step 2 queue (STEP25_GATE=ON, backfill on automatic failures), then Step 3.
...
4. Post a one-line group note: "Feed job $FEED_JOB_ID finished on $PROJECT_SLUG." Then end the turn — do not wait for input beyond any Step 2.5 gate inside the run.
