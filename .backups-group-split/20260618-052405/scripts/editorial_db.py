...
def claim_next_feed_job(*, db_path: str = DEFAULT_DB_PATH) -> FeedJob | None:
    """Atomically promote the oldest queued job to running if none is running."""
    init_db(db_path)
    with _connect(db_path) as conn:
        running = conn.execute(
            "SELECT id FROM feed_jobs WHERE status = 'running' LIMIT 1"
        ).fetchone()
        if running:
            return None
        row = conn.execute(
            "SELECT * FROM feed_jobs WHERE status = 'queued' ORDER BY id ASC LIMIT 1"
        ).fetchone()
        if not row:
            return None
        cur = conn.execute(
        ...
        if cur.rowcount != 1:
            return None
        fresh = conn.execute("SELECT * FROM feed_jobs WHERE id = ?", (int(row["id"]),)).fetchone()
