...
**Last updated:** 2026-07-28 (Writer Pre-Flight verification checklist & template path resolution harmonization)  
...
| LLM backend | Bifrost Vertex proxy (`local-bifrost` provider in `openclaw.json`) |
...
|----------|---------|-----------|-------|---------------|
| `main` | (default) | `workspace/` | gemini-3.1-flash-lite | Not in pipeline |
| `orchestrator` | Nexus | `workspace-orchestrator/` | gemini-3.5-flash | Controller (Steps 0–3) |
| `researcher` | Scout | `workspace-researcher/` | gemini-3.5-flash | Step 1a (HEADLINE_SCAN) + Step 2.1 (DEEP_RESEARCH per pick) |
| `picker` | Sieve | `workspace-picker/` | gemini-3.1-flash-lite | Step 1c — Categorize + select N picks |
| `writer` | Quill | `workspace-writer/` | gemini-3.5-flash | Step 2.2 — Write |
| `chart-generator` | Pixel | `workspace-chart-generator/` | gemini-3.1-flash-lite | Step 2.3 (chart sub-step, optional) |
| `creator` | Pixel | `workspace-creator/` | gemini-3.1-flash-lite | Step 2.3 — Feature image (thinking=low) |
...
| Model | `vertex/gemini-3.5-flash` (fallbacks: 2.5-flash, 3.1-flash-lite) |
...
| Model | `vertex/gemini-3.1-flash-lite` (fallback: 2.5-flash) |
| Pipeline step | 1c |
...
| Date | Change |
|------|--------|
| 2026-07-28 | **Writer Pre-Flight verification checklist & template path resolution harmonization.** Added mandatory 7-point Pre-Flight checklist to `workspace-writer/SOUL.md` and itemized checkboxes to `skills/write-article/SKILL.md` (`PLAN:` -> `PRE-FLIGHT PLAN:` with `- [ ]` syntax) to enforce 1st-turn validation compliance on SEO title, slug tokens, meta description keyword, hook keyword, anchor links, and section/word budgets. Standardized `writer.template_path` across `projects/*.json` to relative canonical paths (`workspace-writer/templates/...`) resolved by `project_config.py`. Updated `DRAINER_LEASE_STALE_MIN` to 15m in `dispatch_feed_jobs.py`. |
| 2026-07-08 | **Full quiet hours (00:00–06:00 IST) — gateway stays alive.** Three layers: (1) official `news-scanner` `heartbeat.activeHours` 06:00–24:00 Asia/Kolkata in `openclaw.json` (skips ~12 nightly heartbeat LLM calls); (2) `pool_scheduler.py` pauses **all** jobs during quiet window (scanner, feed, dispatch, idle) via shared `quiet_hours.py`; (3) `feed-tap-claimer` plugin blocks project-group Telegram inbound during quiet hours (snooze reply, `handled: true`, no orchestrator). Env: `FEED_QUIET_START_HOUR`/`FEED_QUIET_END_HOUR`/`FEED_QUIET_TIMEZONE`. Restart gateway + `pool_scheduler` required. |
| 2026-07-03 | **Coinnetwork watermark.** `projects/coinnetwork.json` `creator.logo_path` → `assets/logo_Coinnetwork.png` (was shared Coinography `logo.png`). Per-run via existing `generate.sh` + `PROJECT_CONFIG`; no code changes. |
| 2026-07-03 | **Onboarding logo/watermark step.** Required `logo_watermark` step in `onboard_project.py` (after `card_prefix`): uploads a transparent PNG to `assets/logo-{slug}.png`, sets `creator.logo_path` in project JSON; Telegram users reply with File/Photo (plugin resolves newest inbound media via `project-onboarder`); CLI wizard accepts a local path; cancel cleans up orphan logos; `validate_project_config.py` errors if logo file missing or not PNG. |
| 2026-07-03 | **Feed card schedule: 3h + IST quiet hours.** `pool_scheduler.py` / `ensure_scheduler.sh` / `openclaw-pool-scheduler.service`: `FEED_EVERY_MIN` 60→180; feed cards snoozed 00:00–06:00 IST (`FEED_QUIET_START_HOUR`/`FEED_QUIET_END_HOUR`); scanner stays 30m. Restart `pool_scheduler` required. |
| 2026-07-02 | **Fix Telegram `/onboard` routing + menu visibility.** Root causes: (1) `customCommands` + `before_dispatch` did not bypass `requireMention: true` in project groups — plain `/onboard` was dropped before the plugin ran; (2) OpenClaw's 5700-char Telegram menu budget already registers 56 commands, so `/onboard` appended last via `customCommands` never reached `setMyCommands`. Fix: `project-onboarder` now registers `/onboard` and `/cancel` via `api.registerCommand()` (dedicated `bot.command` handler, mention-gate bypass, owner auth); adds a `registerService` menu-recovery step that prepends `/onboard` via Telegram `getMyCommands`/`setMyCommands` if missing; removes redundant `customCommands` from `openclaw.json` and `bind_telegram_group.py`. Onboarding text steps still require reply-to-bot in mention-gated groups. Gateway restart required. |
| 2026-07-02 | **Deterministic end-to-end project onboarding (CLI wizard + Telegram `/onboard`).** New zero-LLM engine adds a complete news-agent project (WordPress verify, category/author curation, Telegram wiring, gateway rebind) in one pass. New files: `wp_rest_client.py` (shared REST auth/categories/users helpers; `sync_wp_categories.py` refactored to use it, behavior unchanged), `sync_wp_authors.py`, `validate_project_config.py` (`--openclaw-sync` / `--scanner-ready` / `--live` checks — all 3 existing projects pass), `onboard_project.py` (state-machine engine: `wizard`/`start`/`step`/`status`/`cancel`/`finalize`/`verify`), `bind_telegram_group.py` (atomic, backed-up `openclaw.json` patch for groups+binding+`/onboard` command, defaults to `--dry-run`, `--apply` restarts gateway + verifies health + runs `ensure_scheduler.sh`), `projects/_template.json` + `projects/presets/{general-crypto,memecoin}.json`, `projects/ONBOARDING_CHECKLIST.md` (pinned manual reference). `editorial_db.py` gained an additive `onboard_sessions` table (mirrors `edit_sessions`) — no existing table/column changed. New plugin `plugins/project-onboarder/` claims `/onboard` + session replies via `before_dispatch` (priority 90, below `feed-tap-claimer`'s 100) so onboarding never wakes the orchestrator LLM; owner-gated via `commands.ownerAllowFrom`/`groupAllowFrom`; run from an **existing bound project group** (not a new dedicated control chat), with the target group's `-100…` id supplied as the first reply. `openclaw.json` updated additively: `plugins.entries/load/installs.project-onboarder` + `channels.telegram.accounts.news.customCommands += /onboard` (backup at `.backups/openclaw-json/`; existing groups/bindings/plugins verified byte-identical after the edit). Gateway restart still required to activate `/onboard` in the command menu and load the plugin — not done automatically as part of this change. `projects/README.md` now documents the automated path as primary; manual 4-step procedure kept as fallback. |
| 2026-07-02 | **Image generation: Pollinations → Gemini via Bifrost.** `generate.sh` now calls `POST /v1/images/generations` with primary `vertex/gemini-3.1-flash-lite-image` (~4s) and fallback `huggingface/hf-inference/black-forest-labs/FLUX.1-schnell`. Removed `POLLINATIONS_API_KEY`. On success writes `publish/image-cost.json`; pricing from `workspace-orchestrator/config/image-model-pricing.json` (gateway may revert `openclaw.json` model entries). `aggregate_run_tokens.py` merges image cost into card total; `build_and_send_card.py` shows `Image (…): ~$X.XX` line. |
| 2026-07-02 | **Writer dash bullet enforcement (same pattern as H1/H3/FAQ).** Template `<constraints>` gains `<bullets marker="- " … never="inline_asterisk,…"/>` in COINOGRAPHY + MEMECOIN. `article_hygiene.py`: detect/fix inline ` * ` pseudo-bullets, `* item` lines, `•`, and numbered `1.` lists (FAQ `**N. Question?**` exempt). `check_article.py`: new `canonical_bullet_lists` rule. `autofix_article.py`: mechanical conversion before validator — no new agent workflow. |
| 2026-07-02 | **Vertex Gemini Flash/Lite migration (Bedrock catalog removed).** Replaced all 11 Bedrock open-weight models in `openclaw.json` `local-bifrost` catalog with 5 Vertex Gemini models via Bifrost: `gemini-3.5-flash`, `gemini-3.1-flash-lite`, `gemini-2.5-flash`, `gemini-2.5-pro`, `gemini-3.1-pro-preview`. Agent assignments use Flash/Lite only — orchestrator/researcher/writer → 3.5-flash; picker/creator/scanner/chart-generator/main → 3.1-flash-lite; shared fallback → 2.5-flash. Pro models registered in catalog but not assigned to any agent. Synced `agents/orchestrator/agent/models.json`. |
| 2026-07-02 | **Coinnetwork project added (third news pipeline site).** New `projects/coinnetwork.json` for `https://coinnetwork.info` (WP user `wizardtokenofficial@gmail.com`, `credentials/wp/coinnetwork.pass`). Synced 59 live WP categories via `sync_wp_categories.py`; 51-slug picker allowlist + `fallback_category_id: 5` (News). Authors: Renu Sharma (1), Marcus Webb (2). Dedicated Telegram group `-1003963851656` (`coinnetwork_news-agent`) wired in `openclaw.json` groups ACL + orchestrator binding with `PROJECT_SLUG=coinnetwork`. Reuses Coinography RSS/template/creator defaults; Drive prefix `Coinnetwork News`; per-project `recent_topics-coinnetwork.json`. Background `--all` scripts auto-discover via `list_available_projects()`. |
...
| 2026-06-18 | **Per-publication Telegram groups (Workstream A).** Each publication now routes to its OWN Telegram group instead of the shared `news-agent` group: Coinography -> `-1003760909509`, MemeCoinist -> `-5532153816` (set via `telegram.group_id` in `projects/<slug>.json`, previously dead config). New `build_and_send_card.resolve_chat_id(project, fallback)` reads the project's `telegram.group_id` and falls back to the global `telegram_card_config.json`. Wired into `send_feed_card.py` (per-slug chat id in the send loop + `refresh_card` prefers `feed_cards.telegram_group`), `build_and_send_card.py` main (resolves from manifest `project`), and `dispatch_feed_jobs.py` `_edit_card_state` (uses per-card `feed_cards.telegram_group`). `openclaw.json`: added `-5532153816` to the news account `groups` ACL (`requireMention`) + an explicit orchestrator binding. `SOUL.md`: Step 0 captures `$GROUP_CHAT_ID` from project config; new GROUP ROUTING RULE — Step 2.5 gate, Step 2.6 replies, cap/backfill notices, mini-reports, and the feed-job-finished note must be sent to `$GROUP_CHAT_ID` via the `message` tool (isolated FEED_JOB/AUTO_RUN cron sessions can't rely on implicit "last channel" delivery). Inbound feedback already self-routes via the callback `--chat-id`. Verified live: a memecoinist feed card landed in `-5532153816` (message_id 1607). Backups in `.backups-group-split/`. Concurrency/file-isolation (Workstream B) intentionally deferred — serial feed-job lock kept until per-run file isolation lands. |
